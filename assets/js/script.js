...
fetch(url)
    ...
    .then(function(data) {
        ...
        var newUrl = 'https://api.openweathermap.org/data/2.5/onecall?lat=' + data.coord.lat + '&lon=' + data.coord.lon + '&units=standard&appid=' + apiKey;
    ...
    // Display Today's Weather
        ...
        todayEl.innerHTML = 
        '<h2>' + cityUpper + ' – (' + date + ')</h2>' + '<img src="http://openweathermap.org/img/wn/' + data.daily[0].weather[0].icon + '@2x.png">' +
        '<p>Temp: ' + (Math.floor((data.current.temp-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.current.wind_speed + 'MPH</p>' +
        '<p>Humidity: ' + data.current.humidity + '%</p>' +
        '<p>UV Index: <span id="uv">&nbsp;&nbsp;&nbsp;' + data.current.uvi + '&nbsp;&nbsp;&nbsp;</span></p>';
    ...
    // Display The 5 day Forecast
        ...
        '<img src="http://openweathermap.org/img/wn/' + data.daily[1].weather[0].icon + '.png">' +
        '<p>Temp: ' + (Math.floor((data.daily[1].temp.day-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.daily[1].wind_speed + '</p>' +
        '<p>Humidity: ' + data.daily[1].humidity + '%</p>' +
        '';
        // Day 2
        day2.innerHTML = 
        '<h4 class="date">' + date2 + '</h4>' +
        '<img src="http://openweathermap.org/img/wn/' + data.daily[2].weather[0].icon + '.png">' +
        '<p>Temp: ' + (Math.floor((data.daily[2].temp.day-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.daily[2].wind_speed + '</p>' +
        '<p>Humidity: ' + data.daily[2].humidity + '%</p>' +
        '';
        // Day 3
        day3.innerHTML = 
        '<h4 class="date">' + date3 + '</h4>' +
        '<img src="http://openweathermap.org/img/wn/' + data.daily[3].weather[0].icon + '.png">' +
        '<p>Temp: ' + (Math.floor((data.daily[3].temp.day-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.daily[3].wind_speed + '</p>' +
        '<p>Humidity: ' + data.daily[3].humidity + '%</p>' +
        '';
        // Day 4
        day4.innerHTML = 
        '<h4 class="date">' + date4 + '</h4>' +
        '<img src="http://openweathermap.org/img/wn/' + data.daily[4].weather[0].icon + '.png">' +
        '<p>Temp: ' + (Math.floor((data.daily[4].temp.day-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.daily[4].wind_speed + '</p>' +
        '<p>Humidity: ' + data.daily[4].humidity + '%</p>' +
        '';
        // Day 5
        day5.innerHTML = 
        '<h4 class="date">' + date5 + '</h4>' +
        '<img src="http://openweathermap.org/img/wn/' + data.daily[5].weather[0].icon + '.png">' +
        '<p>Temp: ' + (Math.floor((data.daily[5].temp.day-273.15)*1.8)+32) + '°F</p>' +
        '<p>Wind: ' + data.daily[5].wind_speed + '</p>' +
...
$("#historyList").on("click", "li", function() {
  ...
  // Missing Icon Next to Date
  // Need to make rightside only appear when search has been activated
  // UV index color via api?